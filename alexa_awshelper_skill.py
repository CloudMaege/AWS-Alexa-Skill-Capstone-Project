...
class HelpIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        # type: (HandlerInput) -> Response
        speech_text = "You can ask me about your resources in your A W S account!"
...
class CancelOrStopIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        handler_input.response_builder.speak(speech_text).set_card(
            SimpleCard("Good Bye", speech_text))
        return handler_input.response_builder.response
...
class FallbackIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        speech_text = (
            ...
            "You can say things like how many instances are running, or tell me how much I've spent this month!!")
        reprompt = "What resources can I look at for you?"
...
class ResetInstancesIntentHandler(AbstractRequestHandler):
    def can_handle(self, handler_input):
        # type: (HandlerInput) -> bool
        return is_intent_name("ResetInstancesIntent")(handler_input)
    ...
    def handle(self, handler_input):
        ...
        if startCount > 0:
            if startCount == 1:
                speech_text = f"{startCount} shutdown server has been rebooted."
            elif startCount > 1:
                speech_text = f"{startCount} shutdown servers has been rebooted."
        else:
            speech_text = "I could not find any shutdown servers to reboot, While it's great to learn and be curious, don't forget to be frugal and shutdown development resources that when possible!"
...
class DescribeInstanceIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        key = event['request']['intent']['slots']['InstanceList'] 
        # key = this.event.request.intent.slots.InstanceList.value
	    # key=event.get('key')
...
sb.add_request_handler(ResetInstancesIntentHandler())
